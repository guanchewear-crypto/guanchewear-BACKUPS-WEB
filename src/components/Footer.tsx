...
const FooterLink = ({ href, children }: { href: string; children: string }) => <a href={href} className="group relative w-fit text-sm text-text-muted transition-colors hover:text-gold"><span>{children}</span><span className="absolute -bottom-1 left-0 h-px w-0 bg-gold transition-[width] duration-300 group-hover:w-full" /></a>
    ...
    <footer className="relative overflow-hidden bg-bg-primary px-5 pb-8 pt-20 text-white md:px-10 md:pt-28">
      ...
      <div className="relative mx-auto max-w-7xl">
        <div className="grid gap-14 border-b border-gold/10 pb-20 md:grid-cols-2 lg:grid-cols-[1.5fr_1fr_1fr_1fr]">
          ...
          <div><h3 className="mb-6 text-[10px] font-bold tracking-[.25em] text-text-muted">CONTACTO</h3><div className="flex flex-col gap-4"><a href="mailto:guanchewear@gmail.com" className="flex items-center gap-3 text-sm text-text-muted hover:text-gold"><Mail className="h-4 w-4" />guanchewear@gmail.com</a><a href="https://instagram.com/guanchewear" target="_blank" rel="noreferrer" className="flex items-center gap-3 text-sm text-text-muted hover:text-gold"><AtSign className="h-4 w-4" />@guanchewear</a></div></div>
        </div>
        <div className="flex flex-col gap-6 pt-7 text-[10px] tracking-[.16em] text-text-muted sm:flex-row sm:items-center sm:justify-between"><p>© 2026 GUANCHEWEAR ISLAS CANARIAS · EUROPE</p><div className="flex items-center gap-6"><div aria-label="Selector de idioma" className="flex gap-3"><button className="text-gold" type="button">ES</button><span>/</span><button className="transition-colors hover:text-gold" type="button">EN</button></div><button type="button" aria-label="Volver arriba" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} className="border border-border-subtle p-3 text-white transition-colors hover:bg-gold hover:text-text-contrast"><ArrowUp className="h-4 w-4" /></button></div></div>
      </div>
    </footer>
  )
}
