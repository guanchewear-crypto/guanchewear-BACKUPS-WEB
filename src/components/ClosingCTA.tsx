...
export default function ClosingCTA() {
  return (
    <section className="relative flex min-h-[90dvh] items-center overflow-hidden bg-bg-primary px-5 py-24 text-text-primary md:px-10" aria-labelledby="closing-title">
      ...
      <motion.div aria-hidden="true" initial={{ x: '-110%' }} whileInView={{ x: '110%' }} viewport={{ once: true }} transition={{ duration: 1.8, delay: .7 }} className="absolute inset-y-0 z-10 w-px bg-gradient-to-b from-transparent via-gold/70 to-transparent shadow-[0_0_30px_rgba(212,168,83,0.5)]" />
      ...
      <div className="relative z-30 mx-auto w-full max-w-7xl text-center">
        <h2 id="closing-title" className="mx-auto max-w-5xl text-4xl leading-[1.03] tracking-[-.045em] md:text-7xl lg:text-8xl">
        ...
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 1.55 }} className="mt-10 flex flex-col justify-center gap-3 sm:flex-row">
          <a href="#crear-diseno" className="inline-flex items-center justify-center gap-3 bg-gold px-7 py-4 text-xs font-bold tracking-[.18em] text-text-contrast transition-transform hover:-translate-y-1 hover:bg-gold-light">CREAR MI DISEÑO <ArrowUpRight className="h-4 w-4" /></a>
          <a href="mailto:guanchewear@gmail.com" className="inline-flex items-center justify-center gap-3 border border-gold/30 px-7 py-4 text-xs font-bold tracking-[.18em] text-gold transition-colors hover:bg-gold hover:text-text-contrast">HABLAR CON NOSOTROS <MessageCircle className="h-4 w-4" /></a>
        </motion.div>
      </div>
    </section>
  )
